...
export default function AdminDashboard() {
  ...
  // ── Chart data ─────────────────────────────────────────────────────────────
  const userDonut = [
    { label:"Trainees", value: stats?.totalTrainees||0,  color:"#7c3aed" },
    { label:"Trainers", value: stats?.totalTrainers||0,  color:"#16a34a" },
    { label:"Admins",   value: Math.max(0,(stats?.totalUsers||0)-(stats?.totalTrainees||0)-(stats?.totalTrainers||0)), color:"#1d4ed8" },
  ];
              ...
              {loading.stats ? <Spinner /> : stats && (
                <>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14 }}>
                    <StatBox label="Total Users"    value={stats.totalUsers}       icon="👥" bg="#eff6ff" color="#1d4ed8" trend={`${stats.totalTrainers} trainers · ${stats.totalTrainees} trainees`} />
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14 }}>
                    <StatBox label="Trainers"       value={stats.totalTrainers}    icon="🎓" bg="#fff7ed" color="#ea580c" />
                    <StatBox label="Trainees"       value={stats.totalTrainees}    icon="📖" bg="#fdf4ff" color="#7c3aed" />
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                    ...
                    <div style={S.card}>
                      ...
                      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                        <thead><tr>{["Title","Trainer","Students"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                        <tbody>
                          {courses.slice(0,5).map(c=>(
                            <tr key={c._id} style={{ borderBottom:"1px solid #f8fafc" }}>
                              ...
                              <td style={S.td}>{c.trainer?.name||"—"}</td>
            ...
            <div>
              ...
              {loading.courses ? <Spinner /> : (
                <div style={S.card}>
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                      <thead><tr style={{ background:"#f8fafc" }}>{["Title","Trainer","Category","Level","Price","Students","Zoom","Action"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                      <tbody>
                        {courses.length===0 ? (
              ...
              {loading.sessions ? <Spinner /> : (
                <>
                      ...
                      {sessions.filter(s=>s.isLive).map(s=>(
                        <div key={s._id} style={{ background:"#fff", borderRadius:10, padding:"12px 16px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                          <div>
                            ...
                            <p style={{ fontSize:12, color:"#6b7280", margin:"4px 0 0" }}>by {s.trainer?.name||"—"} {s.course?.title?`· ${s.course.title}`:""}</p>
                  ...
                  <div style={S.card}>
                    ...
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                      <thead><tr style={{ background:"#f8fafc" }}>{["Title","Trainer","Course","Date","Duration","Status","Meeting"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                      <tbody>
                        {sessions.length===0 ? (
                          ...
                          <tr key={s._id} style={{ borderBottom:"1px solid #f8fafc" }}>
                            ...
                            <td style={S.td}>{s.trainer?.name||"—"}</td>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                    ...
                    <div>
                      ...
                      <select value={announcement.targetRole} onChange={e=>setAnnouncement({...announcement,targetRole:e.target.value})} style={{ ...S.inp, background:"#fff" }}>
                        ...
                        <option value="trainee">All Trainees Only</option>
                        <option value="trainer">All Trainers Only</option>
                      </select>
                    </div>
                  </div>
...
function RoleBadge({ role }) {
  const m = { admin:{ bg:"#eff6ff", c:"#1d4ed8" }, trainer:{ bg:"#f0fdf4", c:"#16a34a" }, trainee:{ bg:"#fdf4ff", c:"#7c3aed" } };
